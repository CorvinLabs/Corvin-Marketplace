...
L10 filesystem write protection
...
```bash
pip install corvin-plugin-security_compliance_path_gate
```

## Usage

```python
from corvin_plugins import Security_CompliancePath_Gate

plugin = Security_CompliancePath_Gate()
...
```bash
corvin plugin config plugin:buildin-security_compliance-path_gate
```

## Testing

```bash
pytest tests/plugins/test_security_compliance_path_gate.py -v
```
