...
L34 personally identifiable information detection
...
```bash
pip install corvin-plugin-data_processing_pii_detector
```

## Usage

```python
from corvin_plugins import Data_ProcessingPii_Detector

plugin = Data_ProcessingPii_Detector()
...
```bash
corvin plugin config plugin:buildin-data_processing-pii_detector
```

## Testing

```bash
pytest tests/plugins/test_data_processing_pii_detector.py -v
```
