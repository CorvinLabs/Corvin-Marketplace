...
```bash
pip install corvin-plugin-data_processing_anonymization_engine
```

## Usage

```python
from corvin_plugins import Data_ProcessingAnonymization_Engine

plugin = Data_ProcessingAnonymization_Engine()
...
```bash
corvin plugin config plugin:buildin-data_processing-anonymization_engine
```

## Testing

```bash
pytest tests/plugins/test_data_processing_anonymization_engine.py -v
```
