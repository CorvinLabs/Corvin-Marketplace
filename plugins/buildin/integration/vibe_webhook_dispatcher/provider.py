"""Vibe Webhook Dispatcher — CorvinOS-loadable provider (vibe_webhook_dispatcher).
...
(L3+ NotificationBackend: outbound event/webhook delivery); it is a TAXONOMY label here — ``on_load`` deliberately takes NO
...
class VibeWebhookDispatcherPlugin:
    """Infrastructure plugin — always active, not orchestrated by Skills."""

    plugin_id = "vibe_webhook_dispatcher"
    plugin_type = "notification_backend"
    version = "0.1.0"
    display_name = "Vibe Webhook Dispatcher"
...
Plugin = VibeWebhookDispatcherPlugin
