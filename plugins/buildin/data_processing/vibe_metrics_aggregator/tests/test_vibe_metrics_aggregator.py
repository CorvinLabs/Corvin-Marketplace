"""
Unit tests for vibe_metrics_aggregator plugin.

Tests cover VIBE metrics aggregation including:
- Metrics collection
- Session event handling
- Diagnostics reporting
- Error handling
"""

import pytest
from unittest.mock import MagicMock, AsyncMock
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent.parent / "src"))

from vibe_metrics_aggregator import VibeMetricsAggregator


class TestVibeMetricsAggregatorInitialization:
    """Test plugin initialization."""

    @pytest.mark.asyncio
    async def test_init_success(self):
        """Test successful plugin initialization."""
        plugin = VibeMetricsAggregator()
        assert plugin is not None

    @pytest.mark.asyncio
    async def test_initialize_with_context(self):
        """Test initialize method with context."""
        plugin = VibeMetricsAggregator()
        mock_context = MagicMock()
        await plugin.initialize(mock_context)

    @pytest.mark.asyncio
    async def test_shutdown(self):
        """Test plugin shutdown."""
        plugin = VibeMetricsAggregator()
        await plugin.shutdown()


class TestVibeMetricsAggregatorEventHandling:
    """Test event handling functionality."""

    @pytest.mark.asyncio
    async def test_on_vibe_session_event(self):
        """Test VIBE session event handling."""
        plugin = VibeMetricsAggregator()
        mock_context = MagicMock()
        await plugin.initialize(mock_context)

        event = {
            "session_id": "sess_123",
            "event_type": "session_start",
            "timestamp": "2026-09-01T12:00:00Z"
        }

        result = await plugin.on_vibe_session_event(event)
        # Should process event without error

    @pytest.mark.asyncio
    async def test_on_brain_metric(self):
        """Test brain metric event handling."""
        plugin = VibeMetricsAggregator()
        mock_context = MagicMock()
        await plugin.initialize(mock_context)

        metric = {
            "metric_id": "metric_123",
            "metric_type": "latency",
            "value": 125.5,
            "timestamp": "2026-09-01T12:00:00Z"
        }

        result = await plugin.on_brain_metric(metric)
        # Should process metric without error

    @pytest.mark.asyncio
    async def test_handle_multiple_events(self):
        """Test handling multiple events concurrently."""
        plugin = VibeMetricsAggregator()
        mock_context = MagicMock()
        await plugin.initialize(mock_context)

        events = []
        for i in range(5):
            event = {
                "session_id": f"sess_{i}",
                "event_type": "metric",
                "value": i * 100
            }
            result = await plugin.on_vibe_session_event(event)
            events.append(result)


class TestVibeMetricsAggregatorDiagnostics:
    """Test diagnostics functionality."""

    @pytest.mark.asyncio
    async def test_get_diagnostics(self):
        """Test diagnostics retrieval."""
        plugin = VibeMetricsAggregator()
        mock_context = MagicMock()
        await plugin.initialize(mock_context)

        diags = await plugin.get_diagnostics()
        # Should return diagnostic data
        assert diags is not None

    @pytest.mark.asyncio
    async def test_diagnostics_after_events(self):
        """Test diagnostics after processing events."""
        plugin = VibeMetricsAggregator()
        mock_context = MagicMock()
        await plugin.initialize(mock_context)

        event = {
            "session_id": "sess_123",
            "event_type": "metric",
            "value": 100
        }

        await plugin.on_vibe_session_event(event)
        diags = await plugin.get_diagnostics()

        assert diags is not None


class TestVibeMetricsAggregatorErrorHandling:
    """Test error handling."""

    @pytest.mark.asyncio
    async def test_handle_malformed_event(self):
        """Test handling malformed events."""
        plugin = VibeMetricsAggregator()
        mock_context = MagicMock()
        await plugin.initialize(mock_context)

        # Should handle gracefully: an observability hook must never raise
        # into the host's event loop.
        await plugin.on_vibe_session_event(None)

    @pytest.mark.asyncio
    async def test_handle_missing_fields(self):
        """Test handling events with missing fields."""
        plugin = VibeMetricsAggregator()
        mock_context = MagicMock()
        await plugin.initialize(mock_context)

        incomplete_event = {"session_id": "sess_123"}
        # Should handle gracefully


class TestVibeMetricsAggregatorIntegration:
    """Integration tests."""

    @pytest.mark.asyncio
    async def test_plugin_lifecycle(self):
        """Test complete plugin lifecycle."""
        plugin = VibeMetricsAggregator()
        mock_context = MagicMock()

        await plugin.initialize(mock_context)

        event = {
            "session_id": "sess_123",
            "event_type": "test",
            "value": 100
        }
        await plugin.on_vibe_session_event(event)

        diags = await plugin.get_diagnostics()
        assert diags is not None

        await plugin.shutdown()

    @pytest.mark.asyncio
    async def test_multiple_instances(self):
        """Test multiple plugin instances."""
        plugin1 = VibeMetricsAggregator()
        plugin2 = VibeMetricsAggregator()

        mock_context = MagicMock()
        await plugin1.initialize(mock_context)
        await plugin2.initialize(mock_context)

        assert plugin1 is not plugin2


if __name__ == "__main__":
    pytest.main([__file__, "-v"])
